...
export class ImageAnalyticsController {
    ...
    async subjectIdentification(@UploadedFile() file: Express.Multer.File, @Body() aiRequestDto: LlmRequestDto): Promise<any> {
        if (!file) {
        ...
        return this.imageAnalyticsService.analyzeImage(base64Image, aiRequestDto.provider, aiRequestDto.model, aiRequestDto.promptId);
    }
}
